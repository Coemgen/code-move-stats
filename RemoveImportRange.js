...
const RemoveImportRange = (
  ...
  function (DriveApp, MonthlyRun, PropertiesService) {
    ...
    function isImportRangeConnected(sheetSource, rangeSource){
      ...
      if ( valueCellCheck == "#REF!" && formulaCellCheck.indexOf("=IMPORTRANGE") == 0 )
        return false;
      else
        return true;
    }
    ...
    function getImportRangeArgs(vFormula){
      // Gather requirements
      var argImportRange = vFormula.match(/\((.+)\)/g)[0].split(",").map( x => x.replace("(","").replace(")","").replace('"',"").replace('"',""));
    ...
    function setLocalRangeValues(colLetter, rowNumber, sheetSource, rowValues, isSourceConnected){
      ...
      if ( !isSourceConnected )
        sheetSource.getRange(rangeReplace).setValues(rowValues); // Removes IMPORTRANGE formula using values grabbed from target sheet via Server script; Allow access = N
      else 
        sheetSource.getRange(rangeReplace).setValues( sheetSource.getRange(rangeReplace).getValues() ); // Removes IMPORTRANGE formula using values already imported to sheet; Allow access = Y
    ...
    function getAndSetValuesForColumnLetter(sheetSource, colLetter, rowNumber){
      ...
      // Checking B - =IMPORTRANGE("https://docs.google.com/spreadsheets/d/1tDQLgXEH17h4rKO-4OSLCTVDmEan2iSFDmLAx8aH7n4","Totals!B24:AH24")
        ...
        var cellLocal, formulaCell, valueCell;
        [cellLocal, formulaCell, valueCell] = getCellInformation(sheetSource, rangeCell);
        
        if ( !isImportRangeConnected(sheetSource, rangeCell) )
        {
          // Get Values - gathers requirements from formula arguments
          var targetValues = getTargetRangeValuesByImportRangeFormula(formulaCell) // Gather values as the sheet isn't connected; Allow access:N
          
          // Set Values
          setLocalRangeValues(colLetter, rowNumber, sheetSource, targetValues, false)
    ...
    function main(yearToProcess) {
      ...
      const dataFolder = DriveApp.getFolderById(PropertiesService.getScriptProperties().getProperty("dataFolderId"));
      const yearFolder = dataFolder.getFoldersByName(yearToProcess).next();
      
      var yearlyStatsFile = yearFolder.getFilesByName(yearToProcess + "-stats").next();
      var spreadsheetYearlyStats = SpreadsheetApp.open(yearlyStatsFile);
      
      if ( (spreadsheetYearlyStats == null) || (spreadsheetYearlyStats == undefined) )
        throw "Stats sheet was not found.";
      
      var sheetImportedData = spreadsheetYearlyStats.getSheetByName("Imported Data");
